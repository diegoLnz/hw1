...
// Fetch the list of users followed by the authenticated user
$followsResult = Repository::getAll(
    $conn,
    "follows",
    [new WhereCondition("follower_id", SqlOperators::EQUALS, $userId)]
);

if (mysqli_num_rows($followsResult) == 0) {
    $response = setResponse("OK", "Nessun post trovato", 200);
    echo $response->toJson();
    exit();
}

$followedUserIds = [];
while ($row = mysqli_fetch_assoc($followsResult)) {
...
foreach ($followedUserIds as $followedUserId) {
    $result = Repository::getAll(
        $conn,
        "posts",
        [new WhereCondition("user_id", SqlOperators::EQUALS, $followedUserId)]
    );
